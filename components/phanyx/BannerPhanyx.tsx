...
export default function BannerPhanyx({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border bg-white text-slate-950 shadow-sm transition-all dark:bg-slate-950 dark:text-slate-100 ${cores[aviso.cor] ?? cores.roxo
        }`}
    >
      <div
        className={`absolute left-0 top-0 h-full w-2 ${barraCor[aviso.cor] ?? barraCor.roxo
          }`}
      />
