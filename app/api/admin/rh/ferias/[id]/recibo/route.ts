...
export async function GET(
  ...
  try {
    ...
    drawText("RECIBO DE FÉRIAS", 205, 665, 15, true);
    ...
    drawText("Período aquisitivo:", 65, 535, 10, true);
    drawText(
    ...
    drawText("Período de gozo:", 65, 510, 10, true);
    drawText(
    ...
    drawText("Quantidade de dias:", 65, 485, 10, true);
    drawText(`${ferias.dias} dias`, 240, 485, 10);

    drawText("Data de pagamento:", 65, 460, 10, true);
    drawText(dataBR(ferias.dataPagamento), 240, 460, 10);

    rect(65, 340, 465, 90);

    drawText("Descrição", 80, 410, 10, true);
    drawText("Valor", 450, 410, 10, true);

    line(65, 397, 530, 397);

    drawText("Férias", 80, 375, 10);
    drawRight(`R$ ${moeda(ferias.valorFerias)}`, 515, 375, 10);

    drawText("1/3 constitucional", 80, 352, 10);
    drawRight(`R$ ${moeda(ferias.valorTercoConstitucional)}`, 515, 352, 10);

    line(65, 330, 530, 330);

    drawText("Total líquido de férias", 80, 305, 11, true);
    drawRight(`R$ ${moeda(ferias.valorLiquidoFerias)}`, 515, 305, 11, true);

    drawText(
      "Recebi da empresa acima identificada a importância líquida discriminada neste recibo,",
      65,
      250,
      9
    );
    drawText(
      "referente às férias do período informado, dando plena quitação deste pagamento.",
      65,
      235,
      9
    );

    drawText("Local e data: _________________________________", 65, 190, 10);

    line(330, 125, 510, 125);
    drawText("Assinatura do Funcionário", 350, 110, 9);
    ...
    return new NextResponse(Buffer.from(bytes), {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="${limparArquivo(
          `recibo-ferias-${ferias.funcionario.nome}.pdf`
        )}"`,
      },
    });
  } catch (error: any) {
    return NextResponse.json(
      { error: error?.message || "Erro ao gerar recibo de férias." },
      { status: 500 }
    );
  }
}