...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error("Erro FAL recorte profissional:", falError);

      return NextResponse.json(
        {
          error: "ERRO_IA",
          mensagem:
            "A IA profissional não conseguiu remover o fundo dessa imagem. O crédito foi devolvido.",
        },
        { status: 500 }
      );
    }
  } catch (error) {
    console.error("Erro geral recorte profissional:", error);
