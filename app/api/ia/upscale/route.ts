...
export async function POST(req: NextRequest) {
  ...
  image_url: imageUrl,
      ...
      if (!imagemMelhorada) {
        throw new Error("A FAL não retornou uma imagem.");
      }
      ...
      console.error("Erro FAL upscale:", falError);

      return NextResponse.json(
        {
          error: "ERRO_IA",
          mensagem:
            "A IA não conseguiu melhorar essa imagem. O crédito foi devolvido.",
        },
        { status: 500 }
      );
    }
  } catch (error) {
    console.error("Erro geral upscale IA:", error);
