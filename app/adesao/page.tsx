...
function getDetalhePlano(plano: string) {
  const p = plano.toUpperCase();

  if (p === "ESSENCIAL") {
    return "mensalidade base + alunos ativos + polos extras, conforme regra comercial do plano";
  }
...
function AdesaoContent() {
  ...
  const trialDias = TRIAL_PADRAO_DIAS;
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (status === "ERRO") {
          clearInterval(interval);
          setErro(
            "Ocorreu um erro ao processar sua adesão. Gere uma nova adesão para continuar."
          );
        }
      } catch (error) {
      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-3 text-4xl font-bold">
            Comece o PHANYX com {trialDias} dias grátis
          </h1>

          <p className="mx-auto mt-4 max-w-2xl text-slate-300">
            Preencha os dados da instituição para liberar o ambiente PHANYX em
            teste gratuito. Sua instituição poderá usar a plataforma por{" "}
            {trialDias} dias sem cobrança inicial. Se não houver cancelamento
            antes do fim do período, a cobrança mensal será iniciada conforme o
            plano escolhido, alunos ativos e polos cadastrados.
          </p>
        </div>

        {pagamentoConfirmado ? (
          <div className="mb-8 rounded-3xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-emerald-200">
            <h3 className="text-xl font-bold">Teste gratuito iniciado ✅</h3>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-3xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-slate-400">
              Plano selecionado:{" "}
              <strong className="text-white">{plano}</strong> • {trialDias}{" "}
              dias grátis. Depois: {getValorPlano(plano)} / mês + cobrança por
              aluno ativo e polos adicionais.
            </p>
            ...
            <div className="mt-8 grid gap-4">
              ...
              <div className="rounded-2xl border border-blue-500/30 bg-blue-500/10 p-5">
                <p className="text-sm font-black uppercase tracking-[0.18em] text-blue-200">
                  Forma de cobrança após os {trialDias} dias grátis
                </p>

                <h3 className="mt-2 text-xl font-black text-white">
                  Escolha como a instituição será cobrada depois do período
                  gratuito
                </h3>

                <p className="mt-3 text-sm leading-7 text-blue-100">
                  Nenhuma cobrança será feita agora. A forma escolhida será
                  usada somente após os {trialDias} dias gratuitos, caso a
                  instituição não cancele antes da primeira cobrança.
                </p>
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-400">
                Nenhuma cobrança será gerada agora. O PHANYX liberará o
                ambiente institucional para uso por {trialDias} dias.
              </p>
              ...
              <div className="mt-6 rounded-2xl border border-slate-700 bg-slate-950 p-5">
                ...
                <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-300">
                  ...
                  <li>✓ Período gratuito de {trialDias} dias</li>
                </ul>
              </div>

              <div className="mt-6 rounded-2xl border border-slate-700 bg-slate-950 p-5">
                <p className="text-sm font-bold text-white">
                  Como será a cobrança depois?
                </p>

                <p className="mt-3 text-sm leading-6 text-slate-300">
                  Depois dos {trialDias} dias gratuitos, se a instituição não
                  cancelar antes da primeira cobrança, será cobrada a
                  mensalidade do plano escolhido. O valor poderá incluir{" "}
                  {getDetalhePlano(plano)}.
                </p>
              </div>

              {pagamentoConfirmado ? (
                <div className="mt-4 rounded-2xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-4 text-sm text-emerald-300">
                  Teste gratuito iniciado com sucesso. Sua instituição está
                  sendo ativada automaticamente.
                </div>
              ) : adesaoId ? (
                <div className="mt-4 rounded-2xl border border-blue-500/30 bg-blue-500/10 px-4 py-4 text-sm text-blue-200">
                  Estamos acompanhando automaticamente sua adesão.
                  <div className="mt-2 text-xs text-blue-300/80">
                    Status atual: {statusPagamento}
                  </div>
                  <div className="mt-1 text-xs text-blue-300/70">
                    Assim que a adesão for concluída, você será redirecionada
                    automaticamente.
                  </div>
                </div>
              ) : null}
            ...
            <div className="rounded-3xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
              ...
              <p className="mt-3 text-sm leading-7 text-slate-400">
                Ao iniciar o teste gratuito, o sistema criará automaticamente a
                instituição, o administrador principal, uma senha temporária e
                enviará os dados de acesso para o email informado.
              </p>
            </div>
          </div>
        </div>
