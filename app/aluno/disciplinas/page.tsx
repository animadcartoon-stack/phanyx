...
type DisciplinaAluno = {
  ...
  codigoBloqueio?: "AULAS_NAO_PUBLICADAS" | null;
...
export default function DisciplinasAluno() {
  const t = useTranslations("StudentSubjects");
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <span className="aluno-pill-legivel inline-flex items-center rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">
            {loading
              ? t("loading")
              : t("count", {
                count: disciplinasMatriculadas.length,
              })}
          </span>
        </div>
      </div>
          ...
          {disciplinasFiltradas.map((disciplina) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="aluno-pill-legivel rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">
                  {t("classesCount", {
                    count: disciplina.totalAulas ?? 0,
                  })}
                </span>
              </div>
