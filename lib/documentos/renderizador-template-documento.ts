...
export type OpcoesRenderizacaoDocumento = {
   ...
   *
   * O renderizador geral mantém somente uma paginação mínima
   * no rodapé do Chromium, sem reservar as faixas grandes do
   * layout padrão.
   */
  layoutConteudoIntegral?: boolean;
...
function montarFooterPaginacaoSimples() {
  return `
    <div
      style="
        width: 100%;
...
export function montarRenderizacaoDocumento(
  ...
  const layoutConteudoIntegral =
    Boolean(
      opcoes.layoutConteudoIntegral
    ) &&
    formatoImpressao ===
      "A4_INTEIRA";
  ...
  /*
   * Resolve a logo inline com o mesmo mecanismo usado
   * pela assinatura. Antes desta correção o marcador
   * __PHANYX_LOGO_INSTITUICAO__ podia permanecer no HTML
   * da prévia/documento.
   */
  conteudo =
